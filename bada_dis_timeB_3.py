...
def apply_fl100_deceleration(decel_segments, new_df, flight_levels, 
    ...
    if fl100_index is not None:
        ...
        if current_cas > 250:
            ...
            for i in range(fl100_index, len(new_df)):
                if new_df.iloc[i]['CAS(kt)'] > 250:
                    new_df.at[new_df.index[i], 'CAS(kt)'] = 250.0
...
def apply_approach_deceleration(decel_segments, new_df, flight_levels, 
    ...
    if fl030_index is not None:
        ...
        if current_cas > 220:
            ...
            for i in range(fl030_index, len(new_df)):
                if new_df.iloc[i]['CAS(kt)'] > 220:
                    new_df.at[new_df.index[i], 'CAS(kt)'] = 220.0
...
def calculate_descent_profile(cruise_fl, target_fl, aircraft_mass, ac_model, 
                            ...
                            include_deceleration=True, print_details=True):
    """
    统一的下降剖面计算函数 - 优化参数调用
    ...
    include_deceleration: bool - 是否包含减速段
    print_details: bool - 是否打印详细信息
    
    Returns:
    tuple: (basic_summary, final_summary, basic_df, final_df, decel_segments)
    
    使用示例:
    # 高速段: 0.80M/310kt，FL100→250kt，FL30→220kt
    calculate_descent_profile(370, 30, 60000, "A320-232", 0.80, 310)
    
    # 中速段: 0.76M/285kt，FL100→250kt，FL40→220kt  
    calculate_descent_profile(370, 30, 60000, "A320-232", 0.76, 285, final_approach_fl=40)
    
    # 低速段: 0.73M/250kt，FL30→220kt（无FL100减速）
    calculate_descent_profile(370, 30, 60000, "A320-232", 0.73, 250)
    
    # 自定义中间减速: 0.73M/250kt，FL50→220kt
    calculate_descent_profile(370, 30, 60000, "A320-232", 0.73, 250, intermediate_cas=220, intermediate_fl=50)
    """
    
    # 转换为原函数的参数格式
    if intermediate_cas and intermediate_fl:
        # 有中间减速段
    ...
    basic_summary, final_summary, basic_df, final_df, decel_segments = calculate_complete_descent_profile(
        ...
        include_deceleration=include_deceleration,
...
# 示例1: 高速段 - 0.80M/310kt，FL100→250kt，FL30→220kt
# basic_summary, final_summary, basic_df, final_df, decel_segments = calculate_descent_profile(
#     cruise_fl=370,
#     target_fl=30,
#     aircraft_mass=60000,
#     ac_model="A320-232",
...
# 示例2: 中速段 - 0.76M/285kt，FL100→250kt，FL40→220kt
# basic_summary, final_summary, basic_df, final_df, decel_segments = calculate_descent_profile(
#     cruise_fl=370,
#     target_fl=30,
#     aircraft_mass=60000,
...
#     descent_mach=0.76,
#     high_cas=285,
#     final_approach_fl=40
# )

# 示例3: 低速段 - 0.73M/250kt，FL30→220kt（无FL100减速，因为250≤250）
# basic_summary, final_summary, basic_df, final_df, decel_segments = calculate_descent_profile(
#     cruise_fl=370,
#     target_fl=30,
#     aircraft_mass=60000,
...
# 示例4: 自定义中间减速 - 0.73M/250kt，FL50→220kt
basic_summary, final_summary, basic_df, final_df, decel_segments = calculate_descent_profile(
    cruise_fl=370,
    target_fl=30,
    aircraft_mass=60000,
